...
//    uint y = uint(aIndex) / 1024u;
    ...
    mat3 cov2d_mat = transpose(T) * Vrk * T;
    vec3 cov2d = vec3(cov2d_mat[0][0], cov2d_mat[1][1], cov2d_mat[0][1]);

    float mid = (cov2d.x + cov2d.y) / 2.0;
    float radius = length(vec2((cov2d.x - cov2d.y) / 2.0, cov2d.z));
    float lambda1 = mid + radius, lambda2 = mid - radius;

    if(lambda2 < 0.0) return;
    vec2 diagonalVector = normalize(vec2(cov2d.z, lambda1 - cov2d.x));
